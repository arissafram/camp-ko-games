...
Or use a local server to avoid iframe restrictions on the game pages:
...
```
camp-ko-games/
├── index.html           home page (stays at repo root for GitHub Pages)
├── games/               finished games (instructor reference)
├── templates/           starter templates per game
├── scratch-pad/         blank canvas for open-ended experimentation
└── web-app/
